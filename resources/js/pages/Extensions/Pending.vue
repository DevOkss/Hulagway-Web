...
import { Clock, FileDown, FileSpreadsheet, PlusCircle, Trash2 } from 'lucide-vue-next';
...
const exportUrl = (format: 'pdf' | 'xlsx') => {
    ...
    params.set('status', 'planned,ongoing');
...
const onYearChange = (e: Event) => {
    ...
    router.get('/extensions/pending', v ? { year: v } : {}, { preserveState: true });
};

const breadcrumbs: BreadcrumbItemType[] = [
    { title: 'Extension Activities', href: '/extensions' },
    { title: 'Pending & Ongoing', href: '/extensions/pending' },
];
...
const canDelete = (activity: Activity) => canManage(activity) && activity.status !== 'completed';
...
const confirmDelete = () => {
    ...
    router.delete(`/extensions/${activityToDelete.value.id}`, {
        onSuccess: () => {
...
<template>
    <Head title="Pending & Ongoing Activities" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-4 p-4 sm:p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="font-heading flex items-center gap-2 text-xl font-semibold sm:text-2xl"><Clock class="size-5 text-amber-500" />Pending & Ongoing</h1>
                    <p class="text-sm text-neutral-500">Activities that are planned or currently ongoing — progress advances with uploads.</p>
                </div>
                <Button v-if="isCoordinator" as-child>
                    <Link href="/extensions/create" class="gap-1.5"><PlusCircle class="size-4" /> New Activity</Link>
                </Button>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div v-if="canExport" class="ml-auto flex gap-2">
                    <Button variant="outline" size="sm" as-child class="gap-1.5"><a :href="exportUrl('pdf')"><FileDown class="size-4" /> PDF (pending)</a></Button>
                    <Button size="sm" as-child class="gap-1.5 bg-brand-gradient text-white"><a :href="exportUrl('xlsx')"><FileSpreadsheet class="size-4" /> Excel (pending)</a></Button>
                </div>
            </div>
            ...
            <div v-for="activity in activities" :key="activity.id" class="hcard-p space-y-3">
                ...
                <div class="space-y-1.5">
                    <div class="flex items-center justify-between text-[11px]">
                        ...
                        <span class="font-semibold" :class="statusTint(activity.status).split(' ')[1]">{{ activity.progress }}% · <span class="capitalize">{{ activity.status }}</span></span>
                    </div>
                    <div class="relative h-2.5 w-full overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
                        <div class="absolute left-0 top-0 h-full rounded-full bg-brand-gradient transition-all" :style="{ width: `${activity.progress}%` }" :title="`${activity.progress}% — ${activity.status}`" />
                    </div>
            ...
            <div v-if="!activities.length" class="rounded-2xl border border-dashed border-amber-300 bg-amber-50 p-10 text-center text-sm text-amber-600">
                No pending or ongoing activities.
            </div>
            ...
            <Dialog v-model:open="deleteDialogOpen">
                <DialogContent class="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This will permanently delete “{{ activityToDelete?.title }}” and all its documents and tasks. This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
